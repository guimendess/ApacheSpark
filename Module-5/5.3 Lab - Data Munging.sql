...
select city, state, purchase_revenue_in_usd from purchaseEvents2
...
--total purchases by day of the week 
select DAY(a.eventDate) as day, SUM(a.purchase_revenue_in_usd) as sumrevenue
from purchaseEvents2 a
GROUP BY day(a.eventDate)
order by day(a.eventDate)
...
select date (eventDate), AVG(a.purchase_revenue_in_usd) as avgrevenue
from purchaseEvents2 a
...
select state, SUM(a.purchase_revenue_in_usd) as sumrevenue
from purchaseEvents2 a
GROUP BY state
order by sumrevenue
